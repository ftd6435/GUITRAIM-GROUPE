...
<head>
    ...
    <title>Vérification Facture</title>
    <style>
...
<div class="wrap">
    <div class="card">
        <div class="top">
            ...
            <div>
                <div class="title">Vérification de la facture</div>
                <div class="muted" style="color: rgba(255,255,255,.85); font-size: 12px;">{{ $invoice->invoice_number }}</div>
            </div>
        </div>
        <div class="content">
            <div class="grid">
                <div class="box">
                    ...
                    <div class="value"><span class="badge">{{ $statusLabel }}</span></div>
                </div>
                <div class="box">
                    <div class="label">Date</div>
                    <div class="value">{{ $invoice->issue_date?->format('d/m/Y') }}</div>
                    @if($invoice->due_date)
                        <div class="muted" style="margin-top: 6px;">Échéance: {{ $invoice->due_date?->format('d/m/Y') }}</div>
                    @endif
                </div>
                <div class="box">
                ...
                <div class="box">
                    ...
                    <div class="value">{{ $invoice->sector?->name }}</div>
                    @if($invoice->quote)
                        <div class="muted" style="margin-top: 6px;">Devis: {{ $invoice->quote->quote_number }}</div>
                    @endif
                </div>
            </div>
            ...
            <div style="display:flex; justify-content:flex-end; gap: 12px; margin-top: 12px; flex-wrap: wrap;">
                <div class="box" style="min-width: 240px;">
                    ...
                    <div class="value">{{ number_format((float)$invoice->total_amount, 0, ',', ' ') }} GNF</div>
                </div>
                <div class="box" style="min-width: 240px;">
                    <div class="label">Payé</div>
                    <div class="value">{{ number_format((float)$invoice->amount_paid, 0, ',', ' ') }} GNF</div>
                    <div class="muted" style="margin-top: 6px;">Solde: {{ number_format(((float)$invoice->total_amount) - ((float)$invoice->amount_paid), 0, ',', ' ') }} GNF</div>
                </div>
            </div>

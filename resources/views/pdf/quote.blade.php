...
<head>
    ...
    <style>
        ...
        .muted { color: #64748b; }
        .title { font-size: 26px; font-weight: 700; letter-spacing: .5px; color: #0b2f4e; margin: 0; }
        .hr { height: 1px; background: #e2e8f0; margin: 16px 0; }
        .header { display: table; width: 100%; }
        .header .left, .header .right { display: table-cell; vertical-align: top; }
        .header .right { width: 42%; }
        .badge { display: inline-block; padding: 3px 10px; border-radius: 999px; font-size: 11px; font-weight: 700; background: #e6f1fb; color: #0b2f4e; }
...
<div class="grid">
    <div class="col box">
        ...
        <div style="margin-top: 8px;" class="muted">
            Secteur: <span class="value" style="font-size: 12px;">{{ $quote->sector?->name }}</span>
        </div>
    </div>
    <div class="col box">
        ...
        @if($quote->client?->tax_id)
            <div style="margin-top: 8px;" class="muted">
                Identifiant: <span class="value" style="font-size: 12px;">{{ $quote->client->tax_id }}</span>
            </div>
        @endif
    </div>
</div>
    ...
    <tbody>
        @foreach($quote->items as $item)
            <tr>
                ...
                <td class="text-right nowrap" style="width: 16%; font-weight: 700;">{{ $formatMoney($item->total) }} GNF</td>
            </tr>
        @endforeach
    </tbody>
</table>
    ...
    <div class="spacer">
        @if($quote->notes)
            <div class="box">
                ...
                <div style="white-space: pre-wrap; line-height: 1.5;">{{ $quote->notes }}</div>
            </div>
        @endif
    </div>
    <div class="summary">
        <div class="box">
            <div class="summary-row">
                ...
                <div class="v">{{ $formatMoney($quote->subtotal) }} GNF</div>
            </div>
            <div class="summary-row">
                <div class="k">Taxes</div>
                <div class="v">{{ $formatMoney($quote->tax_amount) }} GNF</div>
            </div>
            <div class="summary-row" style="margin-top: 10px; border-top: 1px solid #e2e8f0; padding-top: 10px;">
                ...
                <div class="v" style="font-size: 14px; color: #0b2f4e;">{{ $formatMoney($quote->total_amount) }} GNF</div>
            </div>
...
<div class="footer">
    ...
    <div class="content">
        ...
        <div class="r">
            {{ $quote->quote_number }} · {{ $quote->issue_date?->format('d/m/Y') }}
        </div>
    </div>
</div>
</body>
